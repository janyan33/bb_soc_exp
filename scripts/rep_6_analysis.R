...
# Load data in
rep_6_dat <- read.csv("data/bb_soc_exp_rep_6.csv") %>% 
  filter(behaviour != "trial start")

unique(rep_6_dat$focal)


patch_table_focals <- read.csv("data/patch_focal_6.csv")
patch_table_partner <- read.csv("data/patch_partner_6.csv")

rep_6_dat <- rep_6_dat %>% 
  left_join(patch_table_focals, by = "focal") %>% 
  left_join(patch_table_partner, by = "partner") 

unique(rep_6_dat$patch_focal)
...
mount_matrix <- func_mount_mat(rep_6_dat)
insem_matrix <- func_insem_mat(rep_6_dat)
...
id <- tkplot(mount_network, vertex.label.family = "Helvetica")
#coords_6 <- tk_coords(id) # Saves coordinates of trial #1
tk_set_coords(id, coords_6) # Set stored coordinates to trial #1
tkplot.fit.to.screen(id)

id <- tkplot(insem_network, vertex.label.family = "Helvetica")
