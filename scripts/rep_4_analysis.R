...
# Load data in
rep_4_dat <- read.csv("data/bb_soc_exp_rep_4.csv") %>% 
  filter(behaviour != "trial start")

patch_table_focals <- read.csv("data/patch_focal_4.csv")
patch_table_partner <- read.csv("data/patch_partner_4.csv")

rep_4_dat <- rep_4_dat %>% 
  left_join(patch_table_focals, by = "focal") %>% 
  left_join(patch_table_partner, by = "partner") 

unique(rep_4_dat$partner)
