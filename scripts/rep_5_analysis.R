...
# Load data in
rep_5_dat <- read.csv("data/bb_soc_exp_rep_5.csv") %>% 
  filter(behaviour != "trial start")

unique(rep_5_dat$partner)


patch_table_focals <- read.csv("data/patch_focal_5.csv")
patch_table_partner <- read.csv("data/patch_partner_5.csv")

rep_5_dat <- rep_5_dat %>% 
  left_join(patch_table_focals, by = "focal") %>% 
  left_join(patch_table_partner, by = "partner") 

unique(rep_5_dat$patch_focal)
...
func_matrix_to_igraph <- function(matrix, mode, behaviour){
  ...
  # V(igraph)$size <- ifelse(V(igraph)$sex == "Female", 15, out_strength) # USE FOR MOUNT NETWORKS
