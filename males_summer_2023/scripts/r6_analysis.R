...
# Load data in
rep_6_dat <- read.csv("males_summer_2023/data/soc_exp_male_r6.csv") %>% 
  filter(behaviour != "trial start")

patch_table_focals <- read.csv("males_summer_2023/data/patch_tables/patch_focal_r6.csv")
patch_table_partner <- read.csv("males_summer_2023/data/patch_tables/patch_partner_r6.csv")
...
write.csv(rep_6_dat, "rep_6_Rdata.csv")
...
func_matrix_to_igraph <- function(matrix, mode, behaviour){
  ...
  #V(igraph)$size <- strength*4 # USE FOR INSEMINATION NETWORKS
