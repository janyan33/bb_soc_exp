...
# Load data in
rep_3_dat <- read.csv("females/data/raw/soc_exp_fem_r3.csv") %>% 
  filter(behaviour != "trial start")

patch_table_focals <- read.csv("females/data/patch_tables/patch_focal_f3.csv")
patch_table_partner <- read.csv("females/data/patch_tables/patch_partner_f3.csv")
...
func_matrix_to_igraph <- function(matrix, mode, behaviour){
  ...
  V(igraph)$size <- strength*6 # CHANGE MULTIPLIER FOR MOUNT VS. INSEM NETWORKS
